...
def verify_and_fix_party_names(parties: dict, original_text: str) -> dict:
    ...
    corrected["原告"] = repair_party_names(parties.get("原告", "原告"), reference_plaintiffs)
    corrected["被告"] = repair_party_names(parties.get("被告", "被告"), reference_defendants)
    corrected["原告數量"] = count_joined_names(corrected["原告"], "原告")
    corrected["被告數量"] = count_joined_names(corrected["被告"], "被告")
    return corrected


def collect_reference_names(text: str, role: str) -> set[str]:
    reference_names: set[str] = set()
    matches = re.findall(rf"{role}([\u4e00-\u9fff○]{{2,4}})", text)
    invalid = {"受有", "所有", "因本", "部分", "應負", "甲車", "乙車"}
    for name in matches:
...
def repair_party_names(joined_names: str, reference_names: set[str]) -> str:
    if not joined_names or joined_names in {"原告", "被告"}:
        return joined_names
    repaired = []
    for name in [item.strip() for item in joined_names.replace("、", ",").split(",") if item.strip()]:
        repaired.append(repair_single_name(name, reference_names))
    return "、".join(repaired) if repaired else joined_names


def repair_single_name(name: str, reference_names: set[str]) -> str:
    if len(name) != 2:
        return name
    surname = name[0]
    last_char = name[1]
...
def find_role_names(text: str, role: str) -> list[str]:
    matches = re.findall(rf"{role}([甲乙丙丁戊己庚辛壬癸○]{{1,4}}|[\u4e00-\u9fff]{{2,4}})(?=[，。；、:\s])", text)
    invalid = {"受有", "所有", "因本", "部分", "應負", "甲車", "乙車", "受傷情形", "事故發生", "發生緣由", "請求賠償", "事實根據"}
    seen = set()
    results = []
    for value in matches:
        ...
        if not clean or clean in invalid or clean in seen:
            continue
        if any(token in clean for token in ["緣由", "情形", "賠償", "事實", "內容", "車輛", "工作", "學歷"]):
            continue
        seen.add(clean)
