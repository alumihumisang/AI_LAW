...
DEFAULT_OUTPUT_DIR = Path("/home/aru/AI_LAW/new_kg/evaluation_outputs")
...
def parse_xrag_records(paths: list[Path]) -> list[dict]:
    rows = []
    for path in paths:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--xrag-jsonl", nargs="*", default=[], help="One or more XRAG/DKG output jsonl files")
...
def main() -> None:
    ...
    xrag_paths = [Path(path) for path in args.xrag_jsonl]
    taarn_paths = [Path(path) for path in args.taarn_jsonl]

    xrag_rows = parse_xrag_records(xrag_paths)
    taarn_rows = parse_taarn_records(taarn_paths)
    top_k_values = sorted({row["top_k"] for row in xrag_rows + taarn_rows if row["top_k"] > 0})
    if not top_k_values:
        top_k_values = [3]
    gpt_rows = build_gpt_rows(queries, top_k_values)

    bert_scorer = None if args.skip_bertscore else BertScoreProxy(args.bert_model)
    evaluated_rows = evaluate_rows(xrag_rows + taarn_rows + gpt_rows, queries, bert_scorer)
