"""
XRAG Phase 2: build database-level LH/HL severity retrieval trees.
...
INPUT_FILE = BASE_DIR / "phase1_boolean_severity_v1.jsonl"
...
def dominant_feature_group(exp: dict) -> str:
    weights = {
        "fact": float(exp["fact_w"]),
        "injury": float(exp["injury_w"]),
        "compensation": float(exp["comp_w"]),
    }
    priority = {"fact": 0, "injury": 1, "compensation": 2}
    return max(weights, key=lambda group: (weights[group], -priority[group]))
...
def summarize_tree(
    ...
    return {
        ...
        "direction": direction,
        "dominant_feature_group": dominant_feature_group(exp),
        "alpha": exp["fact_w"],
        "beta": exp["injury_w"],
...
def build_tree_for_experiment(
    ...
    feature_group = dominant_feature_group(exp)
    group_indices = arrays["feature_group_indices"][feature_group]
    dominant_features = features[:, group_indices]
    ...
    for start in range(0, n, chunk_size):
        ...
        parent_features = features[start:stop]
        parent_dominant_features = dominant_features[start:stop]
        ...
        df = np.abs(parent_features[:, None, :] - features[None, :, :]).mean(axis=2)
        ds = (
    ...
    for direction, parents, distances, deltas in (
        ...
        for child, parent in enumerate(parents):
            ...
            links.append({
                ...
                "config_code": exp["config_code"],
                "parameter_setting": exp["parameter_setting"],
                "dominant_feature_group": feature_group,
                "direction": direction,
