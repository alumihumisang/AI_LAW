...
請分析並輸出："""
    ...
    def _extract_damage_items_from_text(self, text: str) -> Dict[str, List[Dict]]:
        ...
        for sentence in sentences:
            ...
            plaintiff_patterns = [
                r'原告([^，。；、\s]{2,5})',           # 標準格式
                r'([^，。；、\s]{2,5})醫療費用',        # 從損害項目反推原告
                r'([^，。；、\s]{2,5})交通費',          # 從損害項目反推原告
                r'([^，。；、\s]{2,5})工資損失',        # 從損害項目反推原告
                r'([^，。；、\s]{2,5})慰撫金',          # 從損害項目反推原告
            ]
            ...
            # 精確匹配各種損害類型 - 改善版
            # 醫療費用
            if any(keyword in sentence for keyword in ['醫療費用', '醫療費', '就醫費用']):
                amount_match = re.search(r'(?:醫療費用|醫療費|就醫費用)\s*(\d+(?:,\d{3})*)\s*元', sentence)
                if amount_match:
            ...
            # 交通費
            if any(keyword in sentence for keyword in ['交通費用', '交通費', '往返費用']):
                amount_match = re.search(r'(?:交通費用|交通費|往返費用)\s*(\d+(?:,\d{3})*)\s*元', sentence)
                if amount_match:
                    plaintiff_damages[plaintiff].append({
                        'name': '交通費用',
                        'amount': int(amount_match.group(1).replace(',', '')),
            ...
            # 工作損失
            if any(keyword in sentence for keyword in ['工資損失', '薪資損失', '工作損失', '不能工作', '無法工作', '收入損失']):
                amount_match = re.search(r'(?:損失|請求|受有)\s*(\d+(?:,\d{3})*)\s*元', sentence)
                if amount_match:
                    plaintiff_damages[plaintiff].append({
                        'name': '工作損失',
                        'amount': int(amount_match.group(1).replace(',', '')),
            ...
            # 精神慰撫金
            if any(keyword in sentence for keyword in ['慰撫金', '精神慰撫金', '精神賠償', '精神痛苦']):
                amount_match = re.search(r'(?:慰撫金|精神慰撫金|精神賠償)\s*(\d+(?:,\d{3})*)\s*元', sentence)
                if amount_match:
                    plaintiff_damages[plaintiff].append({
                        'name': '精神慰撫金',
                        'amount': int(amount_match.group(1).replace(',', '')),
