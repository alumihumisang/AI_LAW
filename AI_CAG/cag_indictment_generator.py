...
class CAGIndictmentGenerator:
    ...
    def generate_complete_indictment(self, user_input, similar_cases, legal_basis, amounts_info=None):
        ...
        try:
            ...
            # 從原始輸入中提取事故緣由
            print(f"🔍 user_input前200字符: {user_input[:200]}")
            ...
            if accident_origin_match:
                ...
                print(f"🔍 提取到事故緣由: {accident_origin[:100]}...")
                if not accident_origin.startswith('緣'):
                    accident_origin = f"緣{accident_origin}"
            else:
                print("🔍 未匹配到事故緣由，使用預設值")
                accident_origin = "緣被告駕駛車輛發生交通事故，應負賠償責任。"
            
            # 調試：打印 compensation_section 的前100個字符
            print(f"🔍 compensation_section前100字符: {compensation_section[:100]}")
            
            # 檢查 compensation_section 是否已包含完整結構（必須同時包含事故緣由和法條）
            has_full_structure = (
                compensation_section.startswith("一、") and 
                "二、按「因故意或過失" in compensation_section[:500]
            )
            
            if has_full_structure:
                # 如果已包含完整結構，直接使用
                print("🔍 檢測到完整結構，直接使用")
                result = compensation_section
            else:
                # 否則組合事故緣由、法條和賠償項目
                print("🔍 組合事故緣由、法條和賠償項目")
                
                # 檢查 legal_basis 是否已經包含"二、"前綴
                if legal_basis.startswith("二、"):
                    formatted_legal_basis = legal_basis
                else:
                    formatted_legal_basis = f"二、{legal_basis}"
...
{compensation_section}"""
            ...
            return {'full_indictment': result}, generation_time
    ...
    # This large function (~400 lines) has been replaced by generate_complete_indictment + generate_compensation_adaptive
    # Keeping commented for reference but no longer used in the system
    def __enhanced_template_generation_deprecated(self, accident_facts, kv_cache, model_name, use_rule_based_laws=True, amounts_info=None):
        """增強版模板替換起訴書生成 - 整合豐富描述功能"""
        import re
        ...
        def detect_input_type(text):
            ...
            for pattern in indictment_patterns:
                if re.search(pattern, text):
                    return "completed_indictment"
            
            return "raw_facts"
        
        input_type = detect_input_type(accident_facts)
        print(f"🔍 輸入類型識別: {input_type}")
        
        # 優先使用傳入的amounts_info，避免重複解析
        if amounts_info:
            print("🎯 使用傳入的amounts_info，跳過重複解析")
            if input_type == "completed_indictment":
                # 提取基本信息但不重新解析金額
                extracted_facts = self.extract_basic_info_only(accident_facts)
            else:
                # 使用通用解析器處理任意格式的律師輸入
                print("🔄 使用通用解析器處理輸入...")
                ...
                # 直接使用解析結果生成專業起訴書
                if parsed_items:
                    print(f"✅ 通用解析完成，識別到 {len(parsed_items)} 個有效項目")
                    return self.generate_indictment_from_parsed_items(accident_facts, parsed_items, use_rule_based_laws)
                
                # 轉換為舊格式以保持兼容性（作為後備）
                extracted_facts = self.convert_parsed_items_to_facts_format(parsed_items)
                print(f"✅ 通用解析完成，識別到 {len(parsed_items)} 個有效項目")
        elif input_type == "completed_indictment":
            # 直接從完成的起訴書中解析費用
            extracted_facts = self.parse_completed_indictment(accident_facts)
        else:
            # 使用通用解析器處理任意格式的律師輸入
            print("🔄 使用通用解析器處理輸入...")
            parsed_items = self.universal_parse_lawyer_input(accident_facts)
...
請重新整理上述內容，生成專業且有條理的法律文書段落："""
    ...
    def _extract_valid_claim_amounts(self, text: str) -> list:
        ...
        exclude_keywords = [
            ...
            '所得', '薪資所得', '年收入', '底薪',  # 薪資參考數據 (移除'月薪'避免誤排除)
        ...
        for line in lines:
            ...
            for amt_str in line_amounts:
                try:
                    ...
                    # 檢查金額是否緊跟在薪資描述之後（計算基礎）
                    amt_position = context.find(amt_str + '元')
                    before_amount = context[:amt_position] if amt_position != -1 else context
                    after_amount = context[amt_position:] if amt_position != -1 else ""
                    
                    # 更精確的計算基礎判斷：只有當金額直接跟在薪資描述後且沒有賠償相關詞語時才算計算基礎
                    is_calculation_base = False
                    
                    # 檢查是否為純粹的薪資基礎數據
                    if ('每月薪資為' in before_amount or '事故發生前每月薪資為' in before_amount):
                        # 如果後面沒有損失、賠償相關詞語，則為計算基礎
                        if not any(word in after_amount for word in ['損失', '賠償', '故請求', '請求', '一共', '共計', '總計']):
                            is_calculation_base = True
                    
                    # 每日看護費用的計算基礎
                    if '每日' in before_amount and '計算' in before_amount and '總計' not in after_amount:
                        is_calculation_base = True
                    ...
                    # 調試輸出 (暫時註解避免OOM)
                    # print(f"🔍 【分析】{amount:,}元 - 計算基礎:{is_calculation_base}, 有效求償:{is_valid_claim}")
        ...
        # 4. 改進的去重邏輯（參考RAG系統，按項目類型分組）
        damage_items = {}  # 按類型分組：{類型: [金額列表]}
        
        # 檢查是否有格式化標題行
        has_formatted_titles = any(
        ...
        if has_formatted_titles:
                ...
                if (re.match(r'^[（][一二三四五六七八九十][）]', line.strip()) or 
                    ...
                    for amt_str in line_amounts:
                        try:
                            amount = int(amt_str)
                            if amount >= 100:  # 排除小額
                                damage_type = self._classify_damage_type(line)
            ...
            unique_amounts = list(set(amounts))  # 簡單去重
            print(f"🔍 【備用去重】採用 {len(unique_amounts)} 個不同金額")
            return unique_amounts
    ...
    def _classify_damage_type(self, line: str) -> str:
        """分類損害類型（參考RAG系統邏輯）"""
    ...
    def _categorize_amounts_with_context(self, text: str) -> dict:
        ...
        for amount in valid_amounts:
            # 在文本中找到這個金額的上下文
            context = self._find_amount_context(text, amount)
            category = self._smart_classify_by_context(context, amount)
            
            # print(f"🔍 分類 {amount:,}元 為 {category}")
        ...
        print(f"🔍 最終分類結果: {len(categorized_items)}個類別")
        for cat, items in categorized_items.items():
            print(f"  {cat}: {len(items)}個項目")
        
        return categorized_items
    
    def _find_amount_context(self, text: str, amount: int) -> str:
        """找到金額在文本中的最相關上下文"""
        import re
        ...
        for pattern in amount_patterns:
            ...
            if match:
                ...
                # 往前找描述性詞語（通常在金額前面）
                before_text = text[:amount_start]
                
                # 找到最近的詞語邊界，通常是逗號或者空格之後
                # 目標：提取 "醫療費用200,000元" 而不是 "原告林肜宇醫療費用200,000元，看護費用30,00"
                word_breaks = [
                    before_text.rfind('，'),  # 逗號分隔
                    before_text.rfind('、'),  # 頓號分隔
                    before_text.rfind(' '),   # 空格分隔
                    before_text.rfind('，'),  # 全形逗號
                ]
                
                # 找到最近的分隔符
                last_break = max([b for b in word_breaks if b != -1] + [-1])
                
                if last_break != -1:
                    context_start = last_break + 1
                else:
                    # 如果沒有找到分隔符，往前找最多20個字符
                    context_start = max(0, amount_start - 20)
                
                # 往後只到金額結束
                amount_end = match.end()
                
                context = text[context_start:amount_end].strip()
                return context
    ...
    def _extract_plaintiffs(self, text: str) -> list:
        """提取文本中的原告姓名"""
        import re
        plaintiffs = []
        ...
        # 如果找不到特殊格式，再用一般格式，但更嚴格過濾
        if not plaintiffs:
            plaintiff_pattern = r'原告([\u4e00-\u9fff]{2,4})(?=[因於騎駕持搭乘坐在之與及、，。：\s])'
            plaintiff_matches = re.findall(plaintiff_pattern, text)
            for name in plaintiff_matches:
                # 過濾掉明顯不是姓名的詞
                if (name not in plaintiffs and 
                    name not in ['損害負', '受傷情', '因本件', '主張其', '休養期', '主張自', '受有左', '受有身', '請求被', 
                                '之權利', '之損害', '所受', '因此', '依法', '故請求', '受有', '所騎', '之機車']):
                    plaintiffs.append(name)
        
        return plaintiffs
    
    def _generate_multi_plaintiff_compensation(self, comp_facts: str, plaintiffs: list, categorized_items: dict) -> str:
        """生成多原告的賠償項目格式"""
        result_lines = ["三、損害項目："]
        
        # 為每個原告分析其相關的賠償項目
        for i, plaintiff in enumerate(plaintiffs):
            plaintiff_items = self._extract_plaintiff_items(comp_facts, plaintiff, categorized_items, plaintiffs)
            
            if plaintiff_items:
                result_lines.append(f"（{self._get_chinese_number(i+1)}）原告{plaintiff}之損害：")
                
                item_counter = 0
                plaintiff_total = 0
                plaintiff_summary = []
                
                for category, items in plaintiff_items.items():
                    if items:
                        item_counter += 1
                        category_total = sum(item['amount'] for item in items)
                        plaintiff_total += category_total
                        
                        category_display = self._get_category_display_name(category)
                        result_lines.append(f"{item_counter}. {category_display}：{category_total:,}元")
                        
                        description = self._generate_item_description(category, items, comp_facts)
                        description = description.replace('原告', f'原告{plaintiff}')
                        result_lines.append(description)
                        
                        plaintiff_summary.append(f"{category_display}{category_total:,}元")
        ...
        for plaintiff in plaintiffs:
            plaintiff_items = self._extract_plaintiff_items(comp_facts, plaintiff, categorized_items)
            if plaintiff_items:
                plaintiff_total = sum(
                    sum(item['amount'] for item in items) 
                    for items in plaintiff_items.values()
                )
                grand_total += plaintiff_total
                ...
                plaintiff_items_list = []
                for category_key, display_name in item_order:
                    if category_key in plaintiff_items:
                        items = plaintiff_items[category_key]
                        category_total = sum(item['amount'] for item in items)
                        plaintiff_items_list.append(f"{display_name}{category_total:,}元")
                
                if plaintiff_items_list:
                    plaintiff_summaries.append({
                        'name': plaintiff,
                        'items': plaintiff_items_list,
                        'total': plaintiff_total
                    })
        
        if plaintiff_summaries:
            chinese_num = self._get_chinese_number(len(plaintiffs) + 1)
            summary_parts = []
            
            for summary in plaintiff_summaries:
                summary_parts.append(f"應賠償原告{summary['name']}之損害，包含{'、'.join(summary['items'])}，總計{summary['total']:,}元")
            
            result_lines.append(f"（{chinese_num}）綜上所陳，被告{'; '.join(summary_parts)}。兩原告之請求金額合計{grand_total:,}元，並自起訴狀副本送達翌日起至清償日止，按年息5%計算之利息。")
        
        return "\n".join(result_lines)

    def _extract_plaintiff_items(self, text: str, plaintiff_name: str, categorized_items: dict, all_plaintiffs: list = None) -> dict:
        """提取特定原告相關的賠償項目"""
        plaintiff_items = {}
        
        # 在文本中找到與此原告相關的段落
        plaintiff_section = self._find_plaintiff_section(text, plaintiff_name)
        print(f"🔍 為{plaintiff_name}分配項目，相關段落長度: {len(plaintiff_section)}")
        
        # 如果只有一個原告，直接分配所有項目
        if all_plaintiffs and len(all_plaintiffs) == 1:
            # 對於單一原告情況，分配所有項目
            for category, items in categorized_items.items():
                plaintiff_items[category] = items
            return plaintiff_items
        
        for category, items in categorized_items.items():
            plaintiff_category_items = []
            
            for item in items:
                item_context = item['description']
                
                # 方法1: 檢查項目描述是否直接包含原告姓名
                if plaintiff_name in item_context:
                    plaintiff_category_items.append(item)
                    continue
                
                # 方法2: 檢查項目是否在該原告的相關段落中（更嚴格的匹配）
                if plaintiff_section and plaintiff_name in plaintiff_section:
                    # 檢查金額是否在包含該原告姓名的句子中
                    relevant_sentences = [s for s in plaintiff_section.split('.') if plaintiff_name in s]
                    if any(str(item['amount']) in sentence or f"{item['amount']:,}" in sentence 
                           for sentence in relevant_sentences):
                        plaintiff_category_items.append(item)
                        continue
                
                # 方法3: 對於無法明確分配的項目，不分配而不是強制分配
                # 這避免了重複分配同一項目給多個原告
            
            if plaintiff_category_items:
                plaintiff_items[category] = plaintiff_category_items
        
        return plaintiff_items
    
    def _find_plaintiff_section(self, text: str, plaintiff_name: str) -> str:
        """找到特定原告相關的文本段落（更廣泛的搜索）"""
        relevant_lines = []
        
        # 方法1: 搜索包含原告姓名的句子及其上下文
        sentences = text.replace('\n', ' ').split('。')
        for i, sentence in enumerate(sentences):
            if plaintiff_name in sentence:
                # 包含前後句子作為上下文
                start = max(0, i-2)
                end = min(len(sentences), i+3)
                relevant_lines.extend(sentences[start:end])
        
        # 方法2: 如果沒找到，嘗試更寬泛的搜索
        if not relevant_lines:
            lines = text.split('\n')
            for i, line in enumerate(lines):
                if plaintiff_name in line:
                    # 包含前後3行作為上下文
                    start = max(0, i-3)
                    end = min(len(lines), i+4)
                    relevant_lines.extend(lines[start:end])
        
        # 方法3: 如果仍然沒找到，返回所有包含姓名的內容
        if not relevant_lines:
            all_lines = text.split('\n')
            relevant_lines = [line for line in all_lines if plaintiff_name in line]
        
        return '\n'.join(set(relevant_lines))  # 去重
    ...
    def generate_compensation_with_smart_classification(self, comp_facts: str) -> str:
        ...
        # 檢查是否有多個原告
        plaintiffs = self._extract_plaintiffs(comp_facts)
        print(f"🔍 識別到原告: {plaintiffs}")
        print(f"🔍 原告數量: {len(plaintiffs)}")
        
        # 調試：顯示原告相關文本段落
        for plaintiff in plaintiffs:
            section = self._find_plaintiff_section(comp_facts, plaintiff)
            print(f"🔍 {plaintiff}相關段落: {section[:100]}...")
        ...
        #     print(f"  {category}: {len(items)}個項目, 總計{total:,}元")
        #     for item in items:
        #         print(f"    - {item['amount']:,}元: {item['description'][:50]}...")
        
        if not categorized_items:
            print("⚠️ 未找到任何分類項目，使用備用方法")
            # 如果有多個原告，即使沒有分類項目也要使用多原告格式
            if len(plaintiffs) > 1:
                print("🔍 多原告情況下使用LLM備用方法但保持分組格式")
                return self._generate_llm_based_compensation_multi_plaintiff(comp_facts, plaintiffs)
            else:
                return self._generate_llm_based_compensation(comp_facts, {'原告': '原告', '被告': '被告'})
        
        # 如果有多個原告，按原告分組生成
        if len(plaintiffs) > 1:
            return self._generate_multi_plaintiff_compensation(comp_facts, plaintiffs, categorized_items)
    ...
    def _generate_item_description(self, category: str, items: list, original_text: str) -> str:
        ...
        # 根據類別生成詳細描述
        if category == '醫療費用':
            return f"原告因本次事故受傷，為治療傷勢而就醫，支出醫療費用{total_amount:,}元。"
        
        elif category == '看護費用':
            # 從原文提取看護期間
            if '2個月' in original_text and '每日看護2200元' in original_text:
                return f"依診斷證明書記載，原告需專人照顧2個月，以每日看護2,200元計算，共計{total_amount:,}元。"
            else:
                return f"原告因傷勢需專人照護，支出看護費用{total_amount:,}元。"
        
        elif category == '工作損失':
            # 從原文提取工作損失詳情
            if total_amount == 324000:
                return f"原告因本次事故受傷，需休養2個月無法工作，依其事故發生前每月薪資162,000元計算，共計{total_amount:,}元。"
            elif total_amount == 3000:
                return f"原告任職於台鼎有限公司，每月薪資30,000元，因本次事故需請假3日無法工作，受有薪資損失{total_amount:,}元。"
            else:
                return f"原告因本次事故受傷無法工作，造成工作收入損失{total_amount:,}元。"
        
        elif category == '交通費用':
            if '計程車' in original_text:
                return f"原告因足部受傷就醫需搭乘計程車，支出交通費用{total_amount:,}元。"
            else:
                return f"原告因就醫及其他必要事務支出交通費用{total_amount:,}元。"
        
        elif category == '車輛修理費':
            if '機車' in original_text:
                return f"原告所騎乘之機車因本次事故受損，修復費用為{total_amount:,}元。"
            else:
                return f"原告因本次事故導致所駕駛之車輛受損，修復費用{total_amount:,}元。"
        
        elif category == '慰撫金':
            if total_amount == 200000:
                return f"原告因本次事故受有傷害，需定期就醫回診，生活受到相當程度影響，受有相當之精神痛苦，爰請求慰撫金{total_amount:,}元。"
            elif total_amount == 50000:
                return f"原告因本次事故受有傷害，需就醫回診，承受身體不適及工作上受到相當程度影響，受有相當之精神痛苦，爰請求慰撫金{total_amount:,}元。"
            else:
                return f"原告因本次車禍造成身體傷害及精神痛苦，請求慰撫金{total_amount:,}元。"
        
        else:
            return f"原告因本次事故受有損害{total_amount:,}元。"
    ...
    def generate_compensation_adaptive(self, comp_facts: str, parties: dict = None) -> str:
        ...
        if parties is None:
            # 自動檢測原告數量
            plaintiffs = self._extract_plaintiffs(comp_facts)
            plaintiff_count = len(plaintiffs) if plaintiffs else 1
            parties = {
                '原告': '、'.join(plaintiffs) if plaintiffs else '原告', 
                '被告': '被告', 
    ...
    def _smart_classify_by_context(self, context: str, amount: int) -> str:
        ...
        # 更嚴格地排除計算基礎 - 擴展判斷範圍
        if ('每月' in context or '每日' in context) and not ('總計' in context or '一共' in context or '合計' in context or '損失為' in context or '賠償' in context):
            return '計算基礎'
        
        # 特別處理薪資基礎數據
        if ('每月薪資為' in context or '月收入薪資為' in context or '月薪為' in context) and not ('損失' in context):
            return '計算基礎'
        ...
        # 關鍵詞分類規則 - 優先匹配更具體的關鍵詞組合
        classification_rules = {
            '工作損失': ['工作損失', '薪資損失', '收入損失', '無法工作', '不能工作', '損失為', '工作之損失', '請假', '停工', '減少收入', '工資', '收入', '工作', '損失'],  # 提高工作損失的優先級
            '看護費用': ['看護費用', '看護', '照護', '協助', '照顧', '專人照顧', '照顧費', '陪伴', '居家照護'],
            '慰撫金': ['慰撫金', '慰撫', '精神', '痛苦', '身心', '請求', '精神慰撫金', '精神損害', '非財產上損害'],
            '醫療費用': ['醫療費用', '醫院', '治療', '急診', '復健', '醫療', '手術', '診療', '醫療暨醫療耗材費用', '門診', '住院', '檢查', '藥品', '醫藥費', '診斷'],
            '交通費用': ['交通費用', '往返', '油資', '過路費', '計程車', '累計', '就醫交通費用', '車資', '運輸', '就診'],
            '車輛修理費': ['車輛', '修理', '修復', '受損', '機車', '汽車', '花費', '修復費用', '機車受損', '維修', '零件', '拖吊'],
            '復健費用': ['復健', '物理治療', '預估', '未來', '後續治療', '持續治療'],
            '生活費用': ['生活', '日常', '生活品質', '不便', '生活上', '日用'],
            '其他費用': ['費用', '支出', '花費', '開銷']
        }
        
        # 計算每個類別的匹配分數，優先考慮最接近金額的關鍵詞
        scores = {}
        amount_str = f'{amount:,}元'
        amount_pos = context.find(amount_str)
        if amount_pos == -1:
            amount_str = f'{amount}元'
            amount_pos = context.find(amount_str)
            ...
            for keyword in keywords:
                if keyword in context:
                    ...
                    if amount_pos != -1:
                        ...
                        if keyword_pos != -1:
                            distance = abs(keyword_pos - amount_pos)
                            if distance < 20:  # 20個字符內
                                base_score += 2
                            elif distance < 50:  # 50個字符內
                                base_score += 1
        ...
        # 特殊處理：交通費用優先權（但需要更嚴格的條件）
        if '交通費用' in scores and scores['交通費用'] > 0:
            # 確認這個金額確實是在交通費用的直接上下文中
            if ('交通費用' in context and 
                (f'{amount:,}元' in context or f'{amount}元' in context) and
                abs(context.find('交通費用') - context.find(f'{amount:,}元' if f'{amount:,}元' in context else f'{amount}元')) < 30):
                return '交通費用'
...
請嚴格按照上述格式和要求，基於原始描述的事實分析並輸出損害項目："""
            ...
            # 多原告或多被告時，使用智能分類方法而不是LLM
            print("🔄 多原告情況，重定向到智能分類方法...")
            return self.generate_compensation_with_smart_classification(comp_facts)
    ...
    def _generate_llm_based_compensation_multi_plaintiff(self, comp_facts: str, plaintiffs: list) -> str:
        """使用LLM為多原告生成賠償項目（智能分類失敗時的備用方法）"""
        ...
        for i, plaintiff in enumerate(plaintiffs):
            # 提取該原告相關的段落
            plaintiff_section = self._find_plaintiff_section(comp_facts, plaintiff)
            
            if plaintiff_section:
                # 使用LLM為單個原告生成賠償項目
                plaintiff_compensation = self._generate_llm_based_compensation(
                    plaintiff_section, 
                    {'原告': plaintiff, '被告': '被告', '原告數量': 1, '被告數量': 1}
                )
                
                # 修改格式以符合多原告結構
                if plaintiff_compensation.startswith("三、損害項目："):
                    plaintiff_compensation = plaintiff_compensation[7:].strip()
                
                # 添加原告標題
                result_lines.append(f"（{self._get_chinese_number(i+1)}）原告{plaintiff}之損害：")
                result_lines.append(plaintiff_compensation)
                result_lines.append("")
