...
class CAGIndictmentGenerator:
    def __init__(self):
        ...
        # 初始化案件分類器
        self.case_classifier = CaseClassifier(self.llm_url, self.model_name)
    ...
    def generate_complete_indictment(self, user_input, similar_cases, legal_basis, amounts_info=None):
        ...
        try:
            ...
            # 步驟1：案件分類分析
            print("🔬 進行案件分類分析...")
            case_analysis = self.case_classifier.analyze_case(user_input)
            case_type = case_analysis['case_type']
            parties_info = case_analysis['parties']
            
            print(f"📋 案件類型: {case_type}")
            print(f"👥 當事人: 原告{parties_info['原告數量']}人、被告{parties_info['被告數量']}人")
            
            # 步驟2：使用案件類型優化自適應方法生成賠償項目部分
            print("🎯 使用自適應方法生成賠償項目...")
            compensation_section = self.generate_compensation_adaptive(user_input, parties_info)
            ...
            if legal_basis.startswith('二、'):
                legal_section = legal_basis
            else:
                legal_section = f"二、{legal_basis}"
...
{compensation_section}"""
    ...
    def display_results(self, user_input, similar_cases, legal_basis, indictment_result, amounts_info):
        ...
        if actual_items_match:
            ...
            if medical_fee != "未知":
                number = self._get_chinese_number(item_counter)
            ...
            # 使用當前計數器作為綜上所陳的編號
            next_number = self._get_chinese_number(item_counter)
...
（{next_number}）綜上所陳，被告應賠償原告之損害，包含{summary_text}，總計{total_amount}，並自起訴狀副本送達翌日起至清償日止，按年息5%計算之利息。"""
        ...
        # }
    # END OF UNUSED FUNCTION: enhanced_template_generation
    ...
    def display_results(self, user_input, similar_cases, legal_basis, indictment_result, amounts_info):
        ...
        if indictment_result:
            # 處理新的字符串格式或舊的字典格式
            if isinstance(indictment_result, str):
                print(indictment_result)
            elif isinstance(indictment_result, dict) and 'full_indictment' in indictment_result:
                print(indictment_result['full_indictment'])
            else:
                print("❌ 起訴書生成失敗")
        else:
            print("❌ 起訴書生成失敗")
    ...
    def generate_flexible_indictment(self, accident_facts, legal_section, accident_origin, total_amount):
        ...
        # 生成完整起訴書
        next_number = self._get_chinese_number(len(lawyer_items) + 1)
        indictment_template = f"""一、{accident_origin}
...
（{next_number}）綜上所陳，被告應賠償原告之損害，包含{summary_items}，總計{total_amount}，並自起訴狀副本送達翌日起至清償日止，按年息5%計算之利息。"""
    ...
    # UNUSED FUNCTION - Commented out during cleanup
    ...
    def generate_compensation_content(self, lawyer_items, injury_info):
        ...
        for item in lawyer_items:
            ...
            chinese_number = self._get_chinese_number(item_counter)
    ...
    def regex_parse_amounts(self, lawyer_input):
        ...
        for item_name, data in items.items():
            if data['amounts']:
                ...
                chinese_number = self._get_chinese_number(counter)
        ...
        for line in lines:
            ...
            for amt_str in line_amounts:
                try:
                    ...
                    # 排除總計/合計金額（這些是匯總，不是具體求償項目）
                    is_total_amount = ('總計' in context or '合計' in context or '小計' in context)
                    
                    # 特殊處理：如果是看護費用的"一共"描述，這是有效的
                    if '一共' in context and ('看護' in context or '照護' in context):
                        is_total_amount = False
                    
                    # 先檢查是否包含有效求償關鍵詞
                    is_valid_claim = any(keyword in context for keyword in valid_claim_keywords)
                    
                    if is_valid_claim and not is_calculation_base and not is_total_amount:
                        # 檢查是否為強制包含項目
                        ...
                        print(f"🔍 【總計金額】{amount:,}元 - 排除總計/合計: {context[:50]}...")
                    else:
    ...
    def generate_compensation_with_smart_classification(self, comp_facts: str, parties_info: dict = None) -> str:
        """使用智能分類結果生成標準的賠償項目格式"""
        
        print("🧠 使用智能分類生成賠償項目...")
        
        # 檢查是否為多名原告，如果是則使用按原告分組的方法
        if parties_info and parties_info.get('原告數量', 1) > 1:
            print("👥 檢測到多名原告，使用按原告分組格式")
            return self._generate_compensation_by_plaintiffs(comp_facts, parties_info)
    ...
    def _generate_compensation_by_plaintiffs(self, comp_facts: str, parties_info: dict) -> str:
        """按原告分組生成賠償項目（適用於多名原告案件）"""
        
        print("👥 按原告分組生成賠償項目...")
        
        # 提取原告姓名列表
        plaintiffs = parties_info.get('原告', '原告').split('、')
        plaintiff_count = len(plaintiffs)
        
        print(f"📋 檢測到 {plaintiff_count} 名原告: {', '.join(plaintiffs)}")
        
        # 使用LLM分析每個原告的損害項目
        plaintiff_damages = self._extract_damages_by_plaintiff(comp_facts, plaintiffs)
        ...
        for i, plaintiff in enumerate(plaintiffs, 1):
            chinese_num = self._get_chinese_number(i)
            
            result_lines.append(f"（{chinese_num}）原告{plaintiff}之損害：")
            
            damages = plaintiff_damages.get(plaintiff, {})
            plaintiff_total = 0
            plaintiff_items = []
            ...
            item_num = 1
            for damage_key, display_name in damage_order:
                if damage_key in damages and damages[damage_key]['amount'] > 0:
                    amount = damages[damage_key]['amount']
                    description = damages[damage_key]['description']
                    
                    result_lines.append(f"{item_num}. {display_name}：{amount:,}元")
                    result_lines.append(description)
                    result_lines.append("")
                    
                    plaintiff_total += amount
                    plaintiff_items.append(f"{display_name}{amount:,}元")
                    item_num += 1
            
            total_amount += plaintiff_total
            if plaintiff_items:
                overall_summary.append(f"原告{plaintiff}之損害{plaintiff_total:,}元")
        
        # 添加總結
        if len(overall_summary) > 1:
            next_num = self._get_chinese_number(plaintiff_count + 1)
            result_lines.append(f"（{next_num}）綜上所陳，被告應賠償{'、'.join(overall_summary)}，總計{total_amount:,}元，並自起訴狀副本送達翌日起至清償日止，按年息5%計算之利息。")
        
        return "\n".join(result_lines)
    
    def _extract_damages_by_plaintiff(self, comp_facts: str, plaintiffs: list) -> dict:
        """提取每個原告的損害項目"""
        
        print("🔍 分析各原告的損害項目...")
        
        # 如果有LLM可用，使用LLM進行精確分析
        if self.llm_available:
            return self._extract_damages_with_llm(comp_facts, plaintiffs)
        else:
            # 使用規則方法作為fallback
            return self._extract_damages_with_rules(comp_facts, plaintiffs)
    
    def _extract_damages_with_llm(self, comp_facts: str, plaintiffs: list) -> dict:
        """使用LLM提取各原告的損害項目"""
        
        plaintiff_list = "、".join(plaintiffs)
        
        prompt = f"""請分析以下車禍案件文本，為每個原告提取其損害項目和金額。
...
3. 描述要完整，說明傷勢和損害情況"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"🤖 LLM分析結果獲取成功")
                return self._parse_llm_damages_result(llm_result, plaintiffs)
            else:
    ...
    def _parse_llm_damages_result(self, llm_result: str, plaintiffs: list) -> dict:
        """解析LLM的損害分析結果"""
        ...
        damage_types = ['醫療費用', '看護費用', '工作損失', '交通費用', '車輛修復費用', '復健費用', '慰撫金']
        
        for line in lines:
            line = line.strip()
            
            # 檢查是否為原告行
            if line.startswith('原告姓名：'):
                plaintiff_name = line.replace('原告姓名：', '').strip()
                if plaintiff_name in plaintiffs or any(p in plaintiff_name for p in plaintiffs):
                    current_plaintiff = plaintiff_name
                    result[current_plaintiff] = {}
                    continue
            
            # 檢查是否為損害項目行
            if current_plaintiff:
                for damage_type in damage_types:
                    if line.startswith(f'{damage_type}：'):
                        parts = line.replace(f'{damage_type}：', '').split('｜')
                        if len(parts) >= 2:
                            amount_str = parts[0].replace('元', '').replace(',', '')
                            description = parts[1]
                            
                            try:
                                amount = int(amount_str) if amount_str.isdigit() else 0
                                result[current_plaintiff][damage_type] = {
                                    'amount': amount,
                                    'description': description if description != '無' else f"原告{current_plaintiff}無此項損害。"
                                }
                            except:
                                result[current_plaintiff][damage_type] = {
                                    'amount': 0,
                                    'description': f"原告{current_plaintiff}無此項損害。"
                                }
                        break
        ...
        for plaintiff in plaintiffs:
            ...
            for damage_type in damage_types:
                if damage_type not in result[plaintiff]:
                    result[plaintiff][damage_type] = {
                        'amount': 0,
                        'description': f"原告{plaintiff}無此項損害。"
                    }
        
        return result
    
    def _extract_damages_with_rules(self, comp_facts: str, plaintiffs: list) -> dict:
        """使用規則方法提取各原告的損害項目（LLM不可用時的fallback）"""
        
        print("⚠️ 使用規則方法分析損害項目...")
        ...
        for category, items in categorized_items.items():
            ...
            avg_amount = total_amount // plaintiff_count if plaintiff_count > 0 else 0
            
            # 將金額分配給每個原告
            for i, plaintiff in enumerate(plaintiffs):
                if category in result[plaintiff]:
                    result[plaintiff][category] = {
                        'amount': avg_amount,
                        'description': f"原告{plaintiff}因本次事故{category}損失{avg_amount:,}元。"
                    }
    ...
    def _generate_item_description(self, category: str, items: list, original_text: str) -> str:
        ...
        if len(items) > 1 and category == '醫療費用':
            ...
            for item in items:
                ...
                # 移除標題如"三、請求賠償的事實根據:"
                desc = re.sub(r'^.*?[一二三四五六七八九十]、[^：]*：?\s*', '', desc)
                desc = desc.strip()
                
                if '醫院' in desc:
                    hospital_match = re.search(r'([^，。\s]*?醫院)', desc)
                    if hospital_match:
            ...
            if detail_lines:
                # 清理格式，移除連續標點符號
                result = f"{base_desc}，包括{'、'.join(detail_lines)}。"
                # 移除連續的"、、"和其他重複標點
                result = re.sub(r'、{2,}', '、', result)  # 移除連續的、
                result = re.sub(r'，{2,}', '，', result)  # 移除連續的，
                result = re.sub(r'。{2,}', '。', result)  # 移除連續的。
                return result
    ...
    def generate_compensation_adaptive(self, comp_facts: str, parties: dict = None) -> str:
        ...
        if self._is_unstructured_text(comp_facts):
            ...
            return self.generate_compensation_with_smart_classification(comp_facts, parties)
        else:
...
請按照標準格式輸出每位原告的損害項目..."""
    ...
    def generate_indictment_from_parsed_items(self, accident_facts, parsed_items, use_rule_based_laws=True):
        ...
        next_number = self._get_chinese_number(next_number_int)
